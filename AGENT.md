...
- **Mode is implicit**: if a scene first-frame image is present → I→V, if only `reference_image_url` is present → reference-guided video, otherwise → T→V. Do not combine first-frame and reference-media inputs in one Ark task. Character sheets, info graphs, turnarounds, and reference boards are reference inputs, not first-frame inputs; for best shot control generate an actual scene/storyboard frame with imagegen first
