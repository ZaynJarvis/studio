...
- `ARK_IMAGE_MODEL=doubao-seedream-5-0-260128` powers `/api/character-design/zone`, which improves one character identity zone at a time from the Design page. The route uses `ARK_IMAGE_PROMPT_MODEL`/`ARK_VLM_TITLE_MODEL` to summarize source/reference images into a tighter prompt, calls Ark `/images/generations`, then uploads the result to Cloud when `CLOUD_REPO_UPLOAD_KEY` or `MCP_TOKEN` is available. `ARK_IMAGE_SIZE` defaults to `1920x1920`; `ARK_IMAGE_TIMEOUT_MS` defaults to `120000`.
