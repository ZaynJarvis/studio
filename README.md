...
4. Use Studio/`create_video_task` for video generation. Pass the actual scene/storyboard frame as `image_url`. Pass character sheets, info graphs, turnarounds, or reference boards as `reference_image_url` so they are sent to Ark with `role=reference_image`, not as the first frame.
